...
class VM_Process1_노이즈시뮬레이터:
    ...
    def VM_Run(self, lamda_PLS, Z, M, showType="type-1"):
        ...
        for z in np.arange(0, Z):
            # if z == 15:
            #     A_p1_change = self.A * 6  # Noise
            #     C_p1_change = self.C * 6 # Noise
            #     d_p1_change = self.d * 6
            #     self.setParemeter(A_p1_change, d_p1_change, C_p1_change)

            # if z >= 11 and z < 17:
            #     lambda_pls = 0.8

            noise_temp = np.random.randint(-10, 10, size=10) / 1
            noise_temp
            t = 0
