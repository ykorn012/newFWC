...
def main():
    ...
    # fdh_graph.plt_show2(40, ez_run[:, 0:1], ez_run[:, 1:2])
    ...
    for i in np.arange(15 * M, 40 * M, 1):
        mape = fdh_graph.mean_absolute_percentage_error(i + 1, y_act[i][0], y_prd[i][0])
        p1_mape_Queue.append(mape)

    print('P1-MAPE (%) : ', np.mean(p1_mape_Queue))
    np.savetxt("output/p1_mape.csv", p1_mape_Queue, delimiter=",", fmt="%.4f")

    np.savetxt("output/Error_VMresult1.csv", Error_VMresult, delimiter=",", fmt="%.4f")
    np.savetxt("output/p1_y_prd.csv", y_prd, delimiter=",", fmt="%.4f")
    np.savetxt("output/p1_y_act.csv", y_act, delimiter=",", fmt="%.4f")
    ...
    fwc_p2_act.DoE_Run(lamda_PLS=0.5, Z=12, M=10, f=Normal_ACTResult)  # DoE Run ACT값으로 가능
    ...
    fdh_graph.plt_show2(40, p2_ez_run[:, 0:1], p2_ez_run[:, 1:2])
    ...
    np.savetxt("output/p2_mape.csv", p2_mape_Queue, delimiter=",", fmt="%.4f")
