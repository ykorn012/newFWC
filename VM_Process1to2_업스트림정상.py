...
def main():
    ...
    fwc_p1_vm.DoE_Run(lamda_PLS=0.5, Z=Z_DoE, M=M)  # DoE Run
    ...
    p1_VM_Output, p1_ACT_Output, p1_ez_run, p1_y_act, p1_y_prd = fwc_p1_vm.VM_Run(lamda_PLS=0.5, Z=Z_VM, M=M)
    ...
    print('Process-1 q1 Every Metrology MAPE (%) : ', np.mean(p1_q1_mape_Queue))
    p1_q1_mape_Queue = []
    ...
    print('Process-1 q1 All MAPE (%) : ', np.mean(p1_q1_mape_Queue))
    ...
    fwc_p2_act.DoE_Run(lamda_PLS=0.5, Z=Z_DoE, M=M, f=p1_ACT_Output)  #DoE Run ACT값으로 가능
    p2_VM_Output, p2_ACT_Output, p2_ez_run, p2_y_act, p2_y_prd = fwc_p2_act.VM_Run(lamda_PLS=0.5, Z=Z_VM, M=M)

    p2_q2_mape_Queue = []
    for z in range(Z_VM):
        mape = fdh_graph.mean_absolute_percentage_error(z + 1, p2_y_act[((z + 1) * M) - 1][1], p2_y_prd[((z + 1) * M) - 1][1])
        p2_q2_mape_Queue.append(mape)
    print('Process-2 q2 Every Metrology MAPE (%) : ', np.mean(p2_q2_mape_Queue))
    ...
    print('Process-2 q2 All MAPE (%) : ', np.mean(p2_q2_mape_Queue))
